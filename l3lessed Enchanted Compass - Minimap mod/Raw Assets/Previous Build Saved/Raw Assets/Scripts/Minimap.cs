...
namespace Minimap
{
    ...
    public class Minimap : MonoBehaviour, IHasModSaveData
    {
        ...
        void Update()
        {
            ...
            //sets the default time scale when the player loads into the world.
            if (!onstartMenu)
                defaultTimeScale = Time.timeScale;

            //ensures the minimap is disabled and building markers removed anytime the player uses mods that fast travel by adjusting time scales.
            if (defaultTimeScale != Time.timeScale)
            {
                minimapBuildingManager.UpdateMarkers(true);
                minimapActive = false;
                return;
            }
            ...
            if (equippableCompass)
            {
                ...
                //if the amulet is not equipped or not a compass, reset null out current equipped compass and turn of minimap.
        ...
        private void PlayerGPS_OnMapPixelChanged(DFPosition mapPixel)
        {
            ...
            //reset changed location trigger.
        ...
        public void SetupBearings()
        {
            ...
            if (fullMinimapMode && !minimapControls.minimapMenuEnabled && Input.GetMouseButton(0))
            {
                ...
                if (Physics.SphereCast(ray, .5f, out hit))
                {
                    ...
                    BuildingMarker hoverOverBuilding = hit.collider.GetComponentInParent<BuildingMarker>();
        ...
        public void SetupPlayerIndicator()
        {
            ...
            if (!gameobjectPlayerMarkerArrow)
            {
                ...
                gameobjectPlayerMarkerArrow.GetComponent<MeshRenderer>().material = new Material(buildingMarkerMaterial);
