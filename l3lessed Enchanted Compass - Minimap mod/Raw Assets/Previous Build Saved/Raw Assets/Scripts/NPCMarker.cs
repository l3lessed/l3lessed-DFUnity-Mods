...
namespace Minimap
{
    public class npcMarker : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            if((mobileNPC == null && mobileEnemy == null && flatNPC == null) || gameObject == null)
            {
            ...
            updateMaterials();
            marker.npcMarkerMaterial = marker.markerObject.GetComponentInChildren<MeshRenderer>().sharedMaterial;
            ...
            marker.markerIcon = GameObject.CreatePrimitive(PrimitiveType.Quad);
            marker.markerIcon.name = "NPC Icon";
            marker.markerIcon.GetComponentInChildren<MeshRenderer>().sharedMaterial = NPCManager.npcIconMaterialDict[marker.markerType];
            marker.npcIconMaterial = marker.markerIcon.GetComponentInChildren<MeshRenderer>().sharedMaterial;
            ...
            if (mobileNPC != null)
            {
                ...
                {
                    textures = NPCManager.guardTextures;
                }
                else
                {
        ...
        void Update()
        {
            ...
            if(timePass > Minimap.MinimapInstance.npcMarkerUpdateInterval)
            {
                ...
                else if (!Minimap.minimapControls.cameraDetectionEnabled && !Minimap.minimapControls.realDetectionEnabled)
                {
                    ...
                    marker.markerIcon.SetActive(Minimap.npcFlatActive[marker.markerType]);
                    return;
                }
        ...
        {
            marker.npcMarkerMaterial = marker.markerObject.GetComponent<MeshRenderer>().sharedMaterial;
            marker.npcMarkerMaterial = NPCManager.npcMaterialDict[marker.markerType];
            ...
            marker.npcMarkerMaterial.SetFloat("_DistortionEnabled", 1f);
            marker.npcMarkerMaterial.SetFloat("_DistortionStrength", 1f);
            marker.npcMarkerMaterial.SetFloat("_DistortionBlend", 0);
            marker.npcMarkerMaterial.SetFloat("_Mode", 3f);
            marker.npcMarkerMaterial.SetFloat("__ColorMode", 3);
            marker.npcMarkerMaterial.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.SrcAlpha);
            marker.npcMarkerMaterial.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.OneMinusSrcAlpha);
            marker.npcMarkerMaterial.SetInt("_ZWrite", 0);
            marker.npcMarkerMaterial.EnableKeyword("_ALPHABLEND_ON");
            marker.npcMarkerMaterial.EnableKeyword("_COLOROVERLAY_ON");
            marker.npcMarkerMaterial.EnableKeyword("EFFECT_BUMP");
            marker.npcMarkerMaterial.renderQueue = 2000;
            marker.npcMarkerMaterial.color = Minimap.iconGroupColors[marker.markerType];
        }
