...
namespace Minimap
{
    public class RainEffect : MonoBehaviour
    {
        ...
        void Start()
        {
            random = Minimap.randomNumGenerator.Next(10, 18) * .01f;
            randomScale = Minimap.randomNumGenerator.Next((int)(Minimap.MinimapInstance.minimapSize * random), (int)(Minimap.MinimapInstance.minimapSize * random));
            randomPosition = new Vector2(Minimap.randomNumGenerator.Next(-120, 120), Minimap.randomNumGenerator.Next(-120, 136));
            currentAnchorPosition = randomPosition;

            newEffect = Minimap.MinimapInstance.CanvasConstructor(false, string.Concat("Rain Effect", textureID), false, false, true, true, false, 1, 1, randomScale, randomScale, new Vector3(0, 0, 0), effectTexture, textureColor, 0);
            ...
            newEffect.GetComponentInChildren<RawImage>().GetComponent<RectTransform>().sizeDelta = new Vector2(randomScale, randomScale);
            ...
            randomEffectDuration = Minimap.randomNumGenerator.Next(4, 8);
            dripSpeed = Minimap.randomNumGenerator.Next(50, 60);
        ...
        void Update()
        {
            ...
            if (effectType == Minimap.EffectType.None)
            {
                ...
                //if raining start rain effect code.
                    if (!GameManager.Instance.IsPlayerInside && GameManager.Instance.WeatherManager.IsRaining || GameManager.Instance.WeatherManager.IsStorming)
                    {
                        ...
                        {
                            rainSpawnInterval = (Minimap.randomNumGenerator.Next(rainSpawnMin, rainSpawnMax) * .01f);
                            //setup and call random to get random texture list #.
                            int currentRainTextureID = Minimap.randomNumGenerator.Next(0, EffectManager.rainTextureList.Count - 1);
                            //setup base texture
            ...
            if (effectTimer < randomEffectDuration)
            {
                ...
                currentAnchorPosition = new Vector2(effectRectTransform.localPosition.x, effectRectTransform.localPosition.y - (dripSpeed * Time.deltaTime));
